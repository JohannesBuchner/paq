...
protected:
  ...
  virtual void encode(FILE* f, int n) const = 0;  // user suplied
...
void Filter::compress(FILE* f, int n) {
  ...
  if (found || tmpn!=reads) {  // bug in Filter, code untransformed
    ...
    rewind(tmp);
    if (filetype==1) printf("-> %d (x86) ", tmpn);
    int t=filetype;
    filetype=0;
    en.compress(t);
    filetype=t;
...
protected:
  void encode(FILE* f, int n) const {  // not executed if filetype is 0
...
public:
  ...
  void encode(FILE* f, int n) const;
  int decode();
};

void ExeFilter::encode(FILE* f, int n) const {
  Array<U8> buf(BLOCK);
